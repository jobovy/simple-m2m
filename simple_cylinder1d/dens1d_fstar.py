...
# calculate vz
pmvconst=4.74047
velz_eps=pmvconst*pmlat_eps/plx_eps
...
i=0
while i < nstar_cs:
  print >>f, "%f %f %f %f %f %f %f" %(xpos_cs[i],ypos_cs[i],zpos_cs[i], \
...
vzmhist_s=np.histogram(zpos_cs,nbin_n,(zmin_s,zmax_s),weights=velz_cs)[0]/nsz_s
vz2mhist_n=np.histogram(zpos_cs,nbin_n,(zmin_n,zmax_n),weights=(velz_cs**2))[0]/nsz_n
vz2mhist_s=np.histogram(zpos_cs,nbin_n,(zmin_s,zmax_s),weights=(velz_cs**2))[0]/nsz_s
# sig_vz
...
# show histogram
plt.scatter(zbin_n,sigvz_n)
plt.scatter(zbin_s,sigvz_s)
plt.xlabel(r"z (kpc)",fontsize=18,fontname="serif")
plt.ylabel(r"Ns",fontsize=18,fontname="serif")
#plt.axis([-0.25,0.25,0.0,100])
plt.xticks(fontsize=18)
